...
- **Templates.** Stdlib `html/template` with a `layout.html` + per-page body (dashboard, admin, login, setup). The template set is parsed once at boot via `//go:embed` of `templates/*.html`; `RELOAD_TEMPLATES=true` makes `templateSet.render` reparse on each request for fast iteration.

...
For template hot-reload during UI work, run with `RELOAD_TEMPLATES=true` so every request re-parses `templates/*.html`.
...
src/
  ...
  static/              embedded static assets (served as-is from memory)
  ...
  templates/           html/template sources (embedded at build time)
    layout.html, dashboard.html, admin.html, login.html, setup.html
```
